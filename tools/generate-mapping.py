...
x.line(3, '<option variable="scratchTurnsPerRev" type="real" min="0.25" max="8" '
          'step="0.25" default="2" label="Scratch turns per revolution">')
x.line(4, "<description>")
x.line(5, "Turns of a 33 1/3 record per turn of the platter, while scratching. "
          "At 1 the platter covers what a 12-inch turntable would; the NS6 "
          "wheel is about half that diameter, so 2 makes a gesture at the rim "
          "travel the same audio. Raise it to scratch faster.")
x.line(4, "</description>")
x.line(3, "</option>")

x.line(3, '<option variable="bendStrength" type="real" min="0.05" max="3" '
          'step="0.05" default="0.5" label="Pitch bend strength">')
x.line(4, "<description>")
x.line(5, "How far the platter bends the pitch when SCRATCH is off, as the "
          "rate offset produced by turning it at one revolution per second. "
          "0.5 means that speed plays the track half again as fast, and the "
          "bend is proportional, so half that speed bends half as hard.")
x.line(4, "</description>")
x.line(3, "</option>")
